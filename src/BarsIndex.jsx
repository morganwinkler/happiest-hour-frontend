        ...
        <div className="row justify-content-center">
          {props.bars.map((bar) => (
            <div key={bar.id} className="card " style={{width: "18rem", margin: "5px"}}>
              <h2 className="card-title" style={{margin: "10px"}}>{bar.name}</h2>
              <img className="card-img-top" src={bar.image_url} style={{maxHeight: "250px"}} />
              <p className="card-text" style={{margin: "5px"}}>{bar.specials}</p>
              <button className="btn btn-primary" onClick={() => props.onShowBar(bar)}>More Info</button>
            </div>
          ))}
        </div>
    </div>
